...
from comcloak.fec.crc import CRCEncoder
from comcloak.fec.scrambling import TB5GScrambler
from comcloak.fec.ldpc import LDPC5GEncoder
from comcloak.nr.utils import calculate_tb_size
...
class TBEncoder(nn.Module):
    r"""TBEncoder(target_tb_size,num_coded_bits,target_coderate,num_bits_per_symbol,num_layers=1,n_rnti=1,n_id=1,channel_type="PUSCH",codeword_index=0,use_scrambler=True,verbose=False,output_dtype=torch.float32,, **kwargs)
    ...
    Input
    -----
        inputs: [...,target_tb_size] or [...,num_tx,target_tb_size], torch.float
    ...
    def __init__(self,
        ...
        self._output_perm = torch.tensor(perm_seq, dtype=torch.int32)
        self._output_perm_inv = torch.argsort(torch.tensor(perm_seq), dim=-1)
    #########################################
    ...
    def forward(self, inputs):
        ...
        input_shapes = list(inputs.shape)
