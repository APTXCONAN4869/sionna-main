...
from comcloak.channel.utils import deg_2_rad
from comcloak.channel.channel_model import ChannelModel
from comcloak import PI
from comcloak.utils.tensors import insert_dims
...
class CDL(ChannelModel):
    ...
    r"""CDL(model, delay_spread, carrier_frequency, ut_array, bs_array, direction, min_speed=0., max_speed=None, dtype=torch.complex64)
    ...
    ut_orientation : `None` or Tensor of size [3], torch.float
        Orientation of the UT. If set to `None`, [:math:`\pi`, 0, 0] is used.
        Defaults to `None`.

    bs_orientation : `None` or Tensor of size [3], torch.float
    ...
    Output
    -------
    a : [batch size, num_rx = 1, num_rx_ant, num_tx = 1, num_tx_ant, num_paths, num_time_steps], torch.complex
        Path coefficients

    tau : [batch size, num_rx = 1, num_tx = 1, num_paths], torch.float
        Path delays [s]
    ...
    def __init__(   self,
        ...
        if ut_orientation is None:
            ut_orientation = torch.tensor([PI, 0.0, 0.0], dtype=real_dtype)
        if bs_orientation is None:
            bs_orientation = torch.zeros([3], dtype=real_dtype)
    ...
    def __call__(self, batch_size, num_time_steps, sampling_frequency):
        ...
        los_zod = torch.tile(self._los_zod, [batch_size, 1, 1])
        distance_3d = torch.zeros([batch_size, 1, 1], dtype=self._real_dtype)
        tx_orientation = torch.tile(insert_dims(self._tx_orientation, 2, 0),
                                 [batch_size, 1, 1])
        rx_orientation = torch.tile(insert_dims(self._rx_orientation, 2, 0),
                                 [batch_size, 1, 1])
        k_factor = torch.tile(self._k_factor, [batch_size, 1, 1])
        ...
        delays = torch.tile(self._delays*self._delay_spread, [batch_size, 1, 1, 1])
        powers = torch.tile(self._powers, [batch_size, 1, 1, 1])
        aoa = torch.tile(self._aoa, [batch_size, 1, 1, 1, 1])
        aod = torch.tile(self._aod, [batch_size, 1, 1, 1, 1])
        zoa = torch.tile(self._zoa, [batch_size, 1, 1, 1, 1])
        zod = torch.tile(self._zod, [batch_size, 1, 1, 1, 1])
        xpr = torch.tile(self._xpr, [batch_size, 1, 1, 1, 1])
    ...
    def powers(self):
        ...
        if self.los:
            ...
            returned_powers = nlos_powers.clone()
            returned_powers[0] = p0
    ...
    def _load_parameters(self, fname):
        ...
        * xpr : XPR in dB
        ...
        powers = torch.tensor(np.power(10.0, np.array(params['powers'])/10.0),
                                                            dtype=self._real_dtype)

        # Normalize powers
        norm_fact = torch.sum(powers)
        powers = powers / norm_fact

        # Loading the angles and angle spreads of arrivals and departure
        c_aod = torch.tensor(params['cASD'],dtype= self._real_dtype)
        aod = torch.tensor(params['aod'],dtype= self._real_dtype)
        c_aoa = torch.tensor(params['cASA'],dtype= self._real_dtype)
        aoa = torch.tensor(params['aoa'], dtype=self._real_dtype)
        c_zod = torch.tensor(params['cZSD'], dtype=self._real_dtype)
        zod = torch.tensor(params['zod'], dtype=self._real_dtype)
        c_zoa = torch.tensor(params['cZSA'],dtype= self._real_dtype)
        zoa = torch.tensor(params['zoa'], dtype=self._real_dtype)
        ...
        if self._los:
            ...
            k_factor = torch.ones((), dtype=self._real_dtype)

            los_aod = torch.zeros((), dtype=self._real_dtype)
            los_aoa = torch.zeros((), dtype=self._real_dtype)
            los_zod = torch.zeros((), dtype=self._real_dtype)
            los_zoa = torch.zeros((), dtype=self._real_dtype)
    ...
    def _generate_rays(self, angles, c):
        ...
        # TR38.901
        basis_vector = torch.tensor([0.0447, -0.0447,
                                    0.1413, -0.1413,
                                    0.2492, -0.2492,
                                    0.3715, -0.3715,
                                    0.5129, -0.5129,
                                    ...
                                    1.1481, -1.1481,
                                    1.5195, -1.5195,
                                    2.1551, -2.1551], dtype=self._real_dtype)

        # Reshape for broadcasting
        # [1, num rays = 20]
        basis_vector = torch.unsqueeze(basis_vector, dim=0)
        # [num clusters, 1]
        angles = torch.unsqueeze(angles, dim=1)
