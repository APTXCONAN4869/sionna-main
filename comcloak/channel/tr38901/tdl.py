...
from comcloak.utils import insert_dims, expand_to_rank, matrix_sqrt, split_dim, flatten_last_dims
...
class TDL(ChannelModel):
    ...
    r"""TDL(model, delay_spread, carrier_frequency, num_sinusoids=20, los_angle_of_arrival=PI/4., min_speed=0., max_speed=None, num_rx_ant=1, num_tx_ant=1, spatial_corr_mat=None, rx_corr_mat=None, tx_corr_mat=None, dtype=torch.complex64)
    ...
    spatial_corr_mat : [num_rx_ant*num_tx_ant,num_rx_ant*num_tx_ant], torch.complex or `None`
    ...
    rx_corr_mat : [num_rx_ant,num_rx_ant], torch.complex or `None`
    ...
    tx_corr_mat : [num_tx_ant,num_tx_ant], torch.complex or `None`
    ...
    Output
    -------
    a : [batch size, num_rx = 1, num_rx_ant = 1, num_tx = 1, num_tx_ant = 1, num_paths, num_time_steps], torch.complex
        Path coefficients
    ...
    def __init__(   self,
        ...
        if spatial_corr_mat is not None:
            ...
            if rx_corr_mat is not None:
                rx_corr_mat = rx_corr_mat.to(self._dtype)
            ...
            if tx_corr_mat is not None:
                tx_corr_mat = tx_corr_mat.to(self._dtype)
    ...
    def __call__(self, batch_size, num_time_steps, sampling_frequency):
        ...
        theta = torch.rand([ batch_size,
                                    ...
                                    1, # num time steps
                                    self._num_sinusoids],
                                    dtype=self._real_dtype)* (2* PI/self._num_sinusoids.to(self._real_dtype))- (PI / self._num_sinusoids.to(self._real_dtype))
        ...
        phi = torch.rand([   batch_size,
                                    ...
                                    self._num_sinusoids],
                                    dtype=self._real_dtype) * (2*PI)-PI
        ...
        delays = torch.tile(delays, [batch_size, 1, 1, 1])
        ...
        if self._spatial_corr_mat_sqrt is not None:
            ...
            #h = flatten_dims(h, 2, torch.rank(h)-2)  # [..., num_rx_ant*num_tx_ant]
            h = flatten_last_dims(h, 2) # [..., num_rx_ant*num_tx_ant]
            h = torch.unsqueeze(h, dim=-1) # [..., num_rx_ant*num_tx_ant, 1]
            ...
            if ( (self._rx_corr_mat_sqrt is not None)
                ...
                if self._tx_corr_mat_sqrt is not None:
                    h = torch.matmul(h, self._tx_corr_mat_sqrt)
                h = h.permute(0,1,5,2,6,3,4)
