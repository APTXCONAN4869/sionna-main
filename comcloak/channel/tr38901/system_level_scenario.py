...
from comcloak import SPEED_OF_LIGHT,PI
from comcloak.channel.utils import sample_bernoulli, rad_2_deg,wrap_angle_0_360
...
class SystemLevelScenario(ABC):
    ...
    def num_clusters_max(self):
        ...
        num_clusters_max = torch.max(torch.tensor([num_clusters_los, num_clusters_nlos,
            num_clusters_o2i]))
    ...
    def spatial_correlation_matrix(self, correlation_distance):
        ...
        """
        spatial_correlation_matrix = torch.math.exp(-self.matrix_ut_distance_2d/
    ...
    def get_param(self, parameter_name):
        ...
        parameter_tensor = torch.zeros(size=[self.batch_size,
                                            self.num_bs,
                                            self.num_ut],
        ...
        if parameter_name in ('muDS', 'sigmaDS', 'muASD', 'sigmaASD', 'muASA',
            ...
            parameter_value_los = torch.maximum(pa_los,
                pb_los - pc_los*torch.log10(fc))
            parameter_value_nlos = torch.maximum(pa_nlos,
                pb_nlos - pc_nlos*torch.log10(fc))
            parameter_value_o2i = torch.maximum(pa_o2i,
                pb_o2i - pc_o2i*torch.log10(fc))
        else:
        ...
        indoor = torch.unsqueeze(self.indoor, dim=1)
        # LoS
        parameter_value_los = parameter_value_los.to(self._dtype.to_real())
        parameter_tensor = torch.where(self.los, parameter_value_los,
            parameter_tensor)
        # NLoS
        parameter_value_nlos = parameter_value_nlos.to(self._dtype.to_real())
        parameter_tensor = torch.where(
            torch.logical_and(torch.logical_not(self.los),
            torch.logical_not(indoor)), parameter_value_nlos,
            parameter_tensor)
        # O2I
        parameter_value_o2i = parameter_value_o2i.to(self._dtype.to_real())
        parameter_tensor = torch.where(indoor, parameter_value_o2i,
            parameter_tensor)
    ...
    def _compute_distance_2d_3d_and_angles(self):
        ...
        * LoS AoA, AoD, ZoA, ZoD for all BS-UT pairs
        ...
        # 3D distances for all BS-UT pairs
        distance_3d = torch.sqrt(torch.sum(torch.square(delta_loc), dim=3))
        ...
        ut_loc_xy_expanded_1 = torch.unsqueeze(ut_loc_xy, dim=1)
        ut_loc_xy_expanded_2 = torch.unsqueeze(ut_loc_xy, dim=2)
    ...
    def _sample_los(self):
        ...
        if self._requested_los is None:
            ...
            los = torch.fill([self.batch_size, self.num_bs, self.num_ut],
                            self._requested_los)

        self._los = torch.logical_and(los,
            torch.logical_not(torch.unsqueeze(self._in_state, dim=1)))
    ...
    def _sample_indoor_distance(self):
        ...
        indoor = self.indoor
        indoor = torch.unsqueeze(indoor, dim=1) # For broadcasting with BS dim
        indoor_mask = torch.where(indoor, torch.tensor(1.0, dtype=self._dtype.to_real()),
            torch.tensor(0.0, dtype=self._dtype.to_real()))

        # Sample the indoor 2D distances for each BS-UT link
        self._distance_2d_in = (torch.rand(size=[self.batch_size,
            self.num_bs, self.num_ut], dtype=self._dtype.to_real())*(self.max_2d_in-self.min_2d_in)+self.min_2d_in)*indoor_mask
    ...
    def _load_params(self):
        ...
        for param_name in self._params_o2i :
            ...
            if isinstance(v, float):
                self._params_o2i[param_name] = torch.tensor(v,
                                                    dtype=self._dtype.to_real())
            elif isinstance(v, int):
                self._params_o2i[param_name] = torch.tensor(v, dtype=torch.int32)
        ...
        for param_name in self._params_los :
            ...
            if isinstance(v, float):
                self._params_los[param_name] = torch.tensor(v,
                                                    dtype=self._dtype.to_real())
            elif isinstance(v, int):
                self._params_los[param_name] = torch.tensor(v, dtype=torch.int32)
        ...
        for param_name in self._params_nlos :
            ...
            if isinstance(v, float):
                self._params_nlos[param_name] = torch.tensor(v,
                                                        dtype=self._dtype.to_real())
            elif isinstance(v, int):
                self._params_nlos[param_name] = torch.tensor(v, dtype=torch.int32)
