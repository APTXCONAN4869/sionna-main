...
class BaseChannelEstimator(ABC, nn.Module):
    ...
    """
    def __init__(self, resource_grid, interpolation_type="nn", interpolator=None, dtype=torch.complex64, **kwargs):
        super().__init__()
...
class LSChannelEstimator(BaseChannelEstimator, nn.Module):
    ...
    def __init__(self, resource_grid, interpolation_type="nn", interpolator=None, dtype=torch.complex64):
        super().__init__(resource_grid, interpolation_type, interpolator, dtype)

    def estimate_at_pilot_locations(self, y_pilots, no):
        # y_pilots : [batch_size, num_rx, num_rx_ant, num_tx, num_streams, num_pilot_symbols], torch.complex
...
#
class BaseChannelInterpolator(ABC, nn.Module):
...
class LinearInterpolator(BaseChannelInterpolator, nn.Module):
    ...
    Input
    -----
    h_hat : [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx, num_pilot_symbols], torch.complex
        Channel estimates for the pilot-carrying resource elements

    err_var : [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx, num_pilot_symbols], torch.complex
        Channel estimation error variances for the pilot-carrying resource elements

    Output
    ------
    h_hat : [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx, num_ofdm_symbols, fft_size], torch.complex
        Channel estimates across the entire resource grid for all
        transmitters and streams

    err_var : Same shape as ``h_hat``, torch.float
    ...
    def __init__(self, pilot_pattern, time_avg=False):
        ...
        # Pre-compute quantities and indices for frequency- and time-domain interpolation
        ##
        ## Frequency-domain interpolation
        ##
        self._x_freq = torch.tensor(np.expand_dims(np.arange(0, mask.shape[-1]), axis=0),
                                    dtype=torch.float32)

        # Permutation indices to shift batch_dims last during gather
        self._perm_fwd_freq = torch.roll(torch.arange(6), -3)

        x_0_freq = torch.zeros_like(mask, dtype=torch.int32)
        x_1_freq = torch.zeros_like(mask, dtype=torch.int32)

        # Set REs of OFDM symbols without any pilot equal to -1 (dummy value)
        x_0_freq[torch.sum(torch.abs(z), dim=-1) == 0] = -1
        x_1_freq[torch.sum(torch.abs(z), dim=-1) == 0] = -1
        ...
        self._x_0_freq = torch.tensor(x_0_freq, dtype=torch.float32)
        self._x_1_freq = torch.tensor(x_1_freq, dtype=torch.float32)
        ...
        self._y_0_freq_ind = torch.tensor(np.reshape(y_0_freq_ind, mask_shape) + 1, dtype=torch.int64)
        self._y_1_freq_ind = torch.tensor(np.reshape(y_1_freq_ind, mask_shape) + 1, dtype=torch.int64)

        ##
        ## Time-domain interpolation
        ##
        self._x_time = torch.tensor(np.expand_dims(np.arange(0, mask.shape[-2]), -1), dtype=torch.float32)
        self._x_time = torch.tensor(expand_to_rank(self._x_time, 7, axis=0),
                               dtype=pilots.dtype)
        # Indices used to gather estimates
        self._perm_fwd_time = torch.roll(torch.arange(7), -3)

        y_0_time_ind = torch.zeros(size=z.shape[:2], dtype=torch.int32)  # Gather indices
        y_1_time_ind = torch.zeros(size=z.shape[:2], dtype=torch.int32)  # Gather indices

        # For each stream
        for a in range(z.shape[0]):
            # Indices of OFDM symbols for which channel estimates were computed
            ofdm_ind = torch.tensor(np.where(torch.sum(torch.abs(z[a]), dim=-1))[0])
        ...
        self._y_0_time_ind = torch.reshape(y_0_time_ind, mask_shape[:-1])
        self._y_1_time_ind = torch.reshape(y_1_time_ind, mask_shape[:-1])

        self._x_0_time = expand_to_rank(self._y_0_time_ind.unsqueeze(-1),
                                                       7, axis=0)
        self._x_0_time = torch.tensor(self._x_0_time, dtype=pilots.dtype)
        self._x_1_time = expand_to_rank(self._y_1_time_ind.unsqueeze(-1),
                                                       7, axis=0)
        self._x_1_time = torch.tensor(self._x_1_time, dtype=pilots.dtype)

        # Other precomputed values
        # Undo permutation of batch_dims for gather
        self._perm_bwd = torch.roll(torch.arange(7), 3)
        ...
        n = torch.sum(torch.abs(torch.reshape(z, mask_shape)), dim=-1, keepdim=True)
        n = torch.sum(n > 0, dim=-2, keepdim=True)
    ...
    def _interpolate_1d(self, inputs, x, x0, x1, y0_ind, y1_ind):
        ...
        slope = torch.nan_to_num((y1 - y0) / torch.tensor(x1 - x0, dtype=y0.dtype), nan=0.0, posinf=0.0, neginf=0.0)
        return torch.tensor((x - x0) * slope + y0, dtype=y0.dtype)

    def _interpolate(self, inputs):
        # Prepare inputs
        # inputs has shape: [k, l, m, num_tx, num_streams_per_tx, num_pilots]
        ...
        if self._time_avg:
            ...
            h_hat_freq = torch.sum(h_hat_freq, dim=-2, keepdim=True)
            h_hat_freq /= torch.tensor(self._num_pilot_ofdm_symbols, dtype = h_hat_freq.dtype)
        ...
        #  ...num_effective_subcarriers, k, l, m]
        h_hat_time = h_hat_freq.permute(*self._perm_fwd_time)
...
class LMMSEInterpolator1D:
    ...
    **Remark**: The interpolation matrix differs across rows as different
    ...
    def __init__(self, pilot_mask, cov_mat, last_step):
        ...
        assert self._cdtype in (torch.complex64, torch.complex128),\
            "`cov_mat` dtype must be one of torch.complex64 or torch.complex128"
        self._rdtype = self._cdtype.real_dtype
        self._rzero = torch.tensor(0.0, dtype=self._rdtype)
        ...
        # Size of the inner and outer dimension.
        ...
        self._add_err_var_indices = torch.tensor(add_err_var_indices, dtype=torch.int32)
        ...
        self._pil_cov_mat = torch.tensor(pil_cov_mat, dtype=self._cdtype)
        ...
        self._b_mat = torch.tensor(b_mat, dtype=self._cdtype)
        ...
        self._pil_loc = torch.tensor(pil_loc, dtype=torch.int32)
        ...
        self._err_var_mat = torch.tensor(err_var_mat, dtype=self._cdtype)
    ...
    def forward(self, h_hat, err_var):
        ...
        pil_cov_mat = torch.tile(pil_cov_mat, [batch_size, num_rx, num_rx_ant,
                                                     1, 1, 1, 1, 1])
        ...
        # [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx,
        #  outer_dim_size, max_num_pil, max_num_pil]
        pil_cov_mat_ = pil_cov_mat.permute(3, 4, 5, 6, 7, 0, 1, 2)
        err_var_ = torch.complex(err_var, self._rzero)
        err_var_ = err_var_.permute(3, 4, 5, 6, 0, 1, 2)
        a_mat = pil_cov_mat_.clone()
        a_mat[tuple(self._add_err_var_indices.t())] += err_var_
        # a_mat = pil_cov_mat_.scatter_(0, self._add_err_var_indices, err_var_)                                    
        a_mat = a_mat.permute(5, 6, 7, 0, 1, 2, 3, 4)
        #
        # Computing B
        #
        ...
        b_mat = b_mat.tile([batch_size, num_rx, num_rx_ant,
                                1, 1, 1, 1, 1])
        #
        ...
        ext_mat = torch.linalg.lstsq(a_mat, b_mat).solution
        ext_mat = ext_mat.permute(0,1,2,3,4,5,7,6).conj()
        ...
        ext_mat = ext_mat.permute(3, 4, 5, 6, 7, 0, 1, 2)
        init_shape = torch.zeros([num_tx, num_tx_stream,
                                outer_dim_size,
                                inner_dim_size+1,
                                inner_dim_size+1,
                                batch_size, num_rx, num_rx_ant])
        init_shape[tuple(self._pil_loc.t())] = ext_mat
        ext_mat = init_shape.permute(5, 6, 7, 0, 1, 2, 3, 4)
        ...
        h_hat = torch.matmul(ext_mat, h_hat)
        ...
        err_var = torch.diagonal(cov_mat)
        err_var_mat = expand_to_rank(self._err_var_mat, 8, 0)
        err_var_mat = err_var_mat.permute(0, 1, 2, 3, 4, 5, 7, 6)
        err_var = err_var - torch.sum(ext_mat * err_var_mat, dim=-1)
        err_var = torch.real(err_var)
        err_var = torch.maximum(err_var, self._rzero)
        ...
        if not self._last_step:
            ...
            # [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx,
            #  outer_dim_size, inner_dim_size, inner_dim_size]
            ext_mat_h = ext_mat.permute(0, 1, 2, 3, 4, 5, 7, 6).conj()
            # First part of the estimate covariance
            # [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx,
            #  outer_dim_size, inner_dim_size, inner_dim_size]
            h_hat_var_1 = torch.matmul(cov_mat, ext_mat_h)
            h_hat_var_1 = h_hat_var_1.permute(0, 1, 2, 3, 4, 5, 7, 6)
            # [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx,
            #  outer_dim_size, inner_dim_size]
            h_hat_var_1 = torch.sum(ext_mat * h_hat_var_1, dim=-1)
            # Second part of the estimate covariance
            # [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx,
            #  outer_dim_size, inner_dim_size]
            err_var_old_c = torch.complex(err_var_old, self._rzero).unsqueeze(-1)
            h_hat_var_2 = err_var_old_c * ext_mat_h
            h_hat_var_2 = h_hat_var_2.permute(0, 1, 2, 3, 4, 5, 7, 6)
            h_hat_var_2 = torch.sum(ext_mat * h_hat_var_2, dim=-1)
            # Variance of h_hat
            # [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx,
            #  outer_dim_size, inner_dim_size]
            ...
            # [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx,
            #  outer_dim_size, inner_dim_size]
            err_var_c = torch.complex(err_var, self._rzero)
            h_var = torch.diagonal(cov_mat)
            s = torch.nan_to_num(2.0 * h_var / (h_hat_var + h_var - err_var_c), nan=0.0, posinf=0.0, neginf=0.0)
            ...
            # [batch_size, num_rx, num_rx_ant, num_tx, num_streams_per_tx,
            #  outer_dim_size, inner_dim_size]
            err_var = s * (s - 1.0) * h_hat_var + (1.0 - s) * h_var + s * err_var_c
            err_var = torch.real(err_var)
            err_var = torch.maximum(err_var, self._rzero)
...
class SpatialChannelFilter:
    ...
    def __init__(self, cov_mat, last_step):
        self._rzero = torch.zeros((), dtype=cov_mat.dtype.real_dtype)
        self._cov_mat = cov_mat
        self._last_step = last_step
        ...
        self._add_diag_indices = torch.tensor(add_diag_indices, dtype=torch.int32)
    ...
    def forward(self, h_hat, err_var):
        ...
        # Convert err_var to complex type with real part being err_var and imaginary part being zero
        err_var = torch.complex(err_var, self._rzero)
        # Keep track of the previous estimation error variances for later use
        err_var_old = err_var

        # Extract cov_mat and transpose it
        cov_mat = self._cov_mat
        cov_mat_t = torch.transpose(cov_mat)
        num_rx_ant = cov_mat.shape[0]
        ...
        # Expand cov_mat to match the dimensions of err_var
        cov_mat = expand_to_rank(cov_mat, err_var.ndim + 1, dim=0)

        # Adding the error variances to the diagonal
        lmmse_mat = cov_mat.expand(err_var.shape + (num_rx_ant,))
        err_var_ = err_var.permute(6, 0, 1, 2, 3, 4, 5)
        lmmse_mat = lmmse_mat.permute(6, 7, 0, 1, 2, 3, 4, 5)
        # lmmse_mat = lmmse_mat.scatter_add(-1, self._add_diag_indices, err_var_)
        lmmse_mat[tuple(self._add_diag_indices.t())] += err_var_
        lmmse_mat = lmmse_mat.permute(2, 3, 4, 5, 6, 7, 0, 1)

        # Inverse of lmmse_mat
        lmmse_mat = matrix_inv(lmmse_mat)
        lmmse_mat = torch.matmul(cov_mat, lmmse_mat)

        ##########################################
        # Apply smoothing
        ##########################################

        # Expand h_hat for matrix multiplication
        h_hat = h_hat.unsqueeze(-1)
        # [..., num_rx_ant]
        h_hat = torch.matmul(lmmse_mat, h_hat).squeeze(-1)
        ...
        cov_mat_t = expand_to_rank(cov_mat_t, lmmse_mat.ndim, dim=0)
        # [..., num_rx_ant]
        err_var = torch.sum(cov_mat_t * lmmse_mat, dim=-1)
        # [..., num_rx_ant]
        err_var = torch.diagonal(cov_mat, dim1=-2, dim2=-1) - err_var
        err_var = torch.real(err_var)
        err_var = torch.maximum(err_var, self._rzero)
        ...
        if not self._last_step:
            ...
            lmmse_mat_h = lmmse_mat.conj().permute(0, 1, 2, 3, 4, 5, 7, 6)
            # First part of the estimate covariance
            h_hat_var_1 = torch.matmul(cov_mat, lmmse_mat_h)
            h_hat_var_1 = h_hat_var_1.permute(0, 1, 2, 3, 4, 5, 7, 6)
            # [..., num_rx_ant]
            h_hat_var_1 = torch.sum(lmmse_mat * h_hat_var_1, dim=-1)
            # Second part of the estimate covariance
            # [..., num_rx_ant, 1]
            err_var_old = err_var_old.unsqueeze(-1)
            # [..., num_rx_ant, num_rx_ant]
            h_hat_var_2 = err_var_old * lmmse_mat_h
            # [..., num_rx_ant, num_rx_ant]
            h_hat_var_2 = h_hat_var_2.permute(0, 1, 2, 3, 4, 5, 7, 6)
            # [..., num_rx_ant]
            h_hat_var_2 = torch.sum(lmmse_mat * h_hat_var_2, dim=-1)
            # Variance of h_hat
            # [..., num_rx_ant]
            h_hat_var = h_hat_var_1 + h_hat_var_2
            # Scaling factor
...
class LMMSEInterpolator(BaseChannelInterpolator): 
    ...
    **Remark:** The order in which frequency interpolation, temporal
    ...
    def __init__(self, pilot_pattern, cov_mat_time, cov_mat_freq,
        ...
        # super(LMMSEInterpolator, self).__init__()
        ...
        inputs_to_rg_indices = self._build_inputs2rg_indices(pilot_mask,
                                                                   num_pilots)
        self._inputs_to_rg_indices = torch.tensor(inputs_to_rg_indices, dtype = torch.int32)
        ...
        for i, o in enumerate(order):
            ...
            # Frequency
            if o == "f":
                interpolator = LMMSEInterpolator1D(self._pilot_mask, cov_mat_freq, last_step=last_step)
                self._pilot_mask = self._update_pilot_mask_interp(self._pilot_mask)
                err_var_mask = torch.tensor(self._pilot_mask == 1, dtype=torch.float32)
            # Time
            elif o == 't':
                self._pilot_mask = self._pilot_mask.permute(0, 1, 3, 2)
                interpolator = LMMSEInterpolator1D(self._pilot_mask, cov_mat_time, last_step=last_step)
                self._pilot_mask = self._update_pilot_mask_interp(self._pilot_mask)
                self._pilot_mask = self._pilot_mask.permute(0, 1, 3, 2)
                err_var_mask = torch.tensor(self._pilot_mask == 1, dtype=torch.float32)
            # Space
    ...
    def _build_pilot_mask(self, pilot_pattern):
        """
        Build for every transmitter and stream a pilot mask indicating which REs are allocated to pilots, data, or not used.
        # 0 -> Data
        # 1 -> Pilot
        # 2 -> Not used
        """
    ...
    def forward(self, h_hat, err_var):
        ...
        # as h_hat
        err_var = torch.broadcast_to(err_var, h_hat.shape)

        # Map channel estimates and error variances to the resource grid
        ...
        #           num_ofdm_symbols, num_effective_subcarriers]
        h_hat = h_hat.permute(3, 4, 5, 0, 1, 2)
        err_var = err_var.permute(3, 4, 5, 0, 1, 2)
        init_shape = torch.zeros([num_tx, num_tx_stream,
                    num_ofdm_symbols,
                    num_effective_subcarriers,
                    batch_size, num_rx, num_rx_ant])
        init_h_hat = init_shape.clone()
        init_err_var = init_shape.clone()
        init_h_hat[tuple(self._inputs_to_rg_indices.t())] = h_hat
        init_err_var[tuple(self._inputs_to_rg_indices.t())] = err_var
        h_hat = init_h_hat.permute(4, 5, 6, 0, 1, 2, 3)
        err_var = init_err_var.permute(4, 5, 6, 0, 1, 2, 3)
...
def tdl_freq_cov_mat(model, subcarrier_spacing, fft_size, delay_spread,
    ...
    dtype : torch.dtype
        ...
        Should be one of `torch.complex64` or `torch.complex128`.
        Defaults to `torch.complex64`.

    Output
    ------
        cov_mat : [fft_size, fft_size], torch.complex
    ...
    return torch.tensor(cov_mat, dtype=dtype)

def tdl_time_cov_mat(model, speed, carrier_frequency, ofdm_symbol_duration,
                     num_ofdm_symbols, los_angle_of_arrival=PI/4., dtype=torch.complex64):
    r"""
    ...
    Output
    ------
        cov_mat : [num_ofdm_symbols, num_ofdm_symbols], torch.complex
