...
chunks = []
frames_per_chunk = 10
for i in range(0, len(waveform_pad_list), frames_per_chunk):
    chunks.append(np.vstack(waveform_pad_list[i:i+frames_per_chunk]))

TxPointN = len(chunks[0])  # 20 frames each chunk
# TxPointN = RxPointN*20
...
feedback2=tx_write_dac_data(tcp_tx, chunks[0], TxPointN)
...
def producer():
    ...
    while not stop_flag.is_set():
        ...
        internal = current_time - last_time
        if internal > 1 :
            with lock:
                if chunk_id != local_chunk_id:
                    local_chunk_id = chunk_id
                    last_time = current_time
                    print(f"[WARN] Next chunk sending: {internal:.3f} s")
                    # feedback1=tx_set_dac_length(tcp_tx, len(chunks[chunk_id%len(chunks)]))
                    # feedback2=tx_write_dac_data(tcp_tx, chunks[chunk_id%len(chunks)], len(chunks[chunk_id%len(chunks)]))
                    # feedback3=tx_set_dac_start(tcp_tx)
                    feedback1=tx_set_dac_length(tcp_tx, len(chunks[chunk_id]))
                    feedback2=tx_write_dac_data(tcp_tx, chunks[chunk_id], len(chunks[chunk_id]))
                    feedback3=tx_set_dac_start(tcp_tx)
    print("[Producer] exit")
# ========================
# 模拟 Consumer（接收机）
...
def consumer():
    ...
    while not stop_flag.is_set():
        try:
            with lock:
                if unpacker.missing_frames() == len(waveform_pad_list)-frames_per_chunk*(chunk_id+1):
                    chunk_id += 1
                    print(f"[WARN] Next chunk will be sended: {chunk_id}")
            t_start = time.time()
