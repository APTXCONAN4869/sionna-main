...
from comcloak.ofdm import ResourceGrid, RemoveNulledSubcarriers, flatten_last_dims, expand_to_rank, matrix_inv
import numpy as np
...
class BaseChannelEstimator(ABC, nn.Module):
    """BaseChannelEstimator(resource_grid, interpolation_type="nn", interpolator=None, dtype=tf.complex64, **kwargs)
...
class NearestNeighborInterpolator(BaseChannelInterpolator):
    ...
    def _interpolate(self, inputs):
        ...
        inputs = torch.transpose(inputs, perm)
        ...
        outputs = torch.transpose(outputs, perm)
