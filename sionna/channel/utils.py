...
def drop_uts_in_sector(batch_size, num_ut, min_bs_ut_dist, isd,
    ...
    ut_loc = tf.convert_to_tensor(np.stack([ut_loc_x, ut_loc_y], axis=-1))
...
def generate_uts_topology(  batch_size,
    ...
    ut_loc = tf.convert_to_tensor(np.concatenate([ut_loc_xy, ut_loc_z], axis=-1))
    ...
    ut_velocities = tf.convert_to_tensor(np.stack([ut_vel_norm * np.cos(ut_vel_angle),
    ...
    ut_orientations = tf.convert_to_tensor(np.stack([ut_bearing, ut_downtilt, ut_slant], axis=-1))
