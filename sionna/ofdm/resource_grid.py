...
# from sionna.ofdm import pilot_pattern # pylint: disable=line-too-long
...
class ResourceGridMapper(Layer):
    ...
    def call(self, inputs):
        ...
        # print(tf.shape(inputs))# [4032, 128]
        # print(tf.shape(self._data_ind))# [4032, 4]
        rg = tf.tensor_scatter_nd_update(template, self._data_ind, inputs)
        # print(rg)
        # print(self._data_ind)
        # print(tf.shape(rg))# [  2   2  14  72 128]
...
class ResourceGridDemapper(Layer):
    ...
    def call(self, y): # pylint: disable=arguments-renamed
        ...
        # print("self._data_ind.shape:",tf.shape(self._data_ind))
        y = tf.gather(y, self._data_ind, batch_dims=2, axis=2)
        # print("after gather:",tf.shape(y))
        # Put batch_dim first
