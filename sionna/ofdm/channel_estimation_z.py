...
class BaseChannelEstimator(ABC, nn.Module):
    ...
    def forward(self, inputs):
        ...
        #  ..., num_pilot_symbols]
        y_pilots = torch.gather(y_eff_flat, dim=-1, index=self._pilot_ind)
...
class LSChannelEstimator(BaseChannelEstimator, nn.Module):
    ...
    def estimate_at_pilot_locations(self, y_pilots, no):
        ...
        no = expand_to_rank(no, h_ls.ndimension(), -1)
        # Expand rank of pilots for broadcasting
        pilots = expand_to_rank(self._pilot_pattern.pilots, h_ls.ndimension(), 0)
...
class NearestNeighborInterpolator(BaseChannelInterpolator):
    ...
    def _interpolate(self, inputs):
        ...
        outputs = torch.gather(inputs, self._gather_ind, 2, batch_dims=2)
...
class LinearInterpolator(BaseChannelInterpolator):
    ...
    def __init__(self, pilot_pattern, time_avg=False):
        ...
        self._x_0_time = expand_to_rank(np.expand_dims(self._y_0_time_ind, -1),
                                                       7, axis=0)
        self._x_0_time = torch.tensor(self._x_0_time, dtype=pilots.dtype)
        self._x_1_time = expand_to_rank(np.expand_dims(self._y_1_time_ind, -1),
                                                       7, axis=0)
    ...
    def _interpolate_1d(self, inputs, x, x0, x1, y0_ind, y1_ind):
        ...
        y0 = torch.gather(inputs, 2, y0_ind.unsqueeze(2).expand(-1, -1, inputs.size(2), -1, -1, -1))
        y1 = torch.gather(inputs, 2, y1_ind.unsqueeze(2).expand(-1, -1, inputs.size(2), -1, -1, -1))
    ...
    def _interpolate(self, inputs):
        ...
        inputs = torch.nn.functional.pad(inputs, self._pad, value=0)
...
class LMMSEInterpolator1D:
    ...
    **Remark**: The interpolation matrix differs across rows as different
    ...
    def forward(self, h_hat, err_var):
        ...
        ext_mat = init_shape.scatter_(0, self._pil_loc, ext_mat)
...
class SpatialChannelFilter:
    ...
    def forward(self, h_hat, err_var):
        ...
        if not self._last_step:
            ...
            lmmse_mat_h = lmmse_mat.conj().transpose(-1, -2)
            # First part of the estimate covariance
    ...
    **Remark:** The order in which frequency interpolation, temporal
    ...
    def __init__(self, pilot_pattern, cov_mat_time, cov_mat_freq,
        ...
        for i, o in enumerate(order):
            ...
            # Time
                ...
                self._pilot_mask = self._pilot_mask.transpose(0, 1, 3, 2)
    ...
    def forward(self, h_hat, err_var):
        ...
        init_shape = torch.zeros([num_tx, num_tx_stream,
                    ...
                    num_effective_subcarriers,
                    batch_size, num_rx, num_rx_ant])
        h_hat = init_shape.scatter_(0, self._inputs_to_rg_indices, h_hat)
        
        err_var = init_shape.scatter_(0, self._inputs_to_rg_indices, err_var)
        h_hat = h_hat.permute(4, 5, 6, 0, 1, 2, 3)
        err_var = err_var.permute(4, 5, 6, 0, 1, 2, 3)
