...
class BaseChannelEstimator(ABC, Layer):
    ...
    def call(self, inputs):
        y, no = inputs
        # print(tf.where(y != 0 + 0j))
        # print("output:\n", tf.boolean_mask(y, y != 0 + 0j))
        # y has shape:
        ...
        # print(tf.where(y_eff_flat != 0 + 0j))
...
class LSChannelEstimator(BaseChannelEstimator, Layer):
    ...
    def estimate_at_pilot_locations(self, y_pilots, no):
        ...
        # np.save('tftensor.npy', y_pilots.numpy())
        # print("output:\n", tf.boolean_mask(self._pilot_pattern.pilots, self._pilot_pattern.pilots != 0 + 0j))
        # print("output:\n", tf.boolean_mask(y_pilots, y_pilots != 0 + 0j))
        # print("output:\n", tf.boolean_mask(h_ls, h_ls != 0 + 0j))
