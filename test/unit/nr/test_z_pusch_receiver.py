...
def run_test(pusch_configs, channel_estimator="perfect", domain="freq", num_rx=1, num_rx_ant=8, batch_size=128, dtype=torch.complex64):
    ...
    def run():
        ...
        if channel_estimator == "perfect":
            b_hat = pusch_receiver([y, h, torch.tensor(0.001, dtype=get_real_dtype(dtype))])
        else:
            b_hat = pusch_receiver([y, torch.tensor(0.001, dtype=get_real_dtype(dtype))])
